namespace Microsoft.Ciqs.Saw.Cli
{
    ...
    class InformationPrinter
    {
        ...
        public void PrintNewSettings()
        {
            Console.WriteLine("SAW configuration has been successfully updated.\n");
            this.parameterPool.ToList().ForEach(p =>
            {
                var value = this.SanitizeSecureString(p.Value);
        ...
        private string SanitizeSecureString(string value, bool sanitizeUnrecognized = false)
        {
            Match match = InformationPrinter.accountKeyRegex.Match(value);   
            if (match.Success)
            {
                return accountKeyRegex.Replace(value, InformationPrinter.accountKeyReplacementString);
            }
            
            if (sanitizeUnrecognized)
            {
