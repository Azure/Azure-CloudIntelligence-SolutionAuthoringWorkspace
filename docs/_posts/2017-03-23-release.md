...
According to [ODBC 3.0 spec](https://msdn.microsoft.com/en-us/library/ms161962.aspx), `[] {}() , ; ? * ! @` are not permitted in OLE DB or ODBC connection strings; If, **bcp.exe** or **sqlcmd.exe** is used in the solution, `odbc` credential control helps to opt out these special characters in credential inputs.
